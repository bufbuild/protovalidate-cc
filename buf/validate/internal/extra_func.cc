...
namespace buf::validate::internal {
    ...
    absl::Status RegisterExtraFuncs(
        ...
        auto isHostnameStatus =
                ...
                CreateAndRegister("isHostname", true, &isHostname, &registry);
        if (!isHostnameStatus.ok()) {
            return isHostnameStatus;
        }
        auto isEmailStatus =
                cel::FunctionAdapter<cel::CelValue, cel::CelValue::StringHolder>::
                CreateAndRegister("isEmail", true, &isEmail, &registry);
        if (!isEmailStatus.ok()) {
            return isHostnameStatus;
        }
        auto isIPStatus =
                cel::FunctionAdapter<cel::CelValue, cel::CelValue::StringHolder>::
                CreateAndRegister("isIP", true, &isIP, &registry);
        if (!isIPStatus.ok()) {
            return isHostnameStatus;
        }
        auto isIPv4Status =
                cel::FunctionAdapter<cel::CelValue, cel::CelValue::StringHolder>::
                CreateAndRegister("isIPv4", true, &isIPv4, &registry);
        if (!isIPv4Status.ok()) {
            return isHostnameStatus;
        }
        auto isIPv6Status =
                cel::FunctionAdapter<cel::CelValue, cel::CelValue::StringHolder>::
                CreateAndRegister("isIPv6", true, &isIPv6, &registry);
        if (!isIPv6Status.ok()) {
            return isHostnameStatus;
        }
