...
class BaseValidator {
 public:
  //  /**
  //   * Validate/check a generic message object with a registered validator for the concrete
  //   message
  //   * type.
  ...
  //    // Polymorphic lookup is used to see if there is a matching concrete validator. If so, call
  //    it.
 ...
 public:
  Validator(std::function<bool(const T&, ValidationMsg*)> check) : check_(check) {
    abstractValidators()[std::type_index(typeid(T))] =
        [this](const google::protobuf::Message& m, ValidationMsg* err) -> bool {
